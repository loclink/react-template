...
export class Loader {
  ...
  loadPages() {
    ...
    for (const pkg of this.root.config.packages) {
      const routeConfigSourceFiles = this.project.addSourceFilesAtPaths(
        pkg.pagePath + '/**/route.config.ts'
      )

      fs.readdirSync(pkg.pagePath)
        // 过滤一些非页面文件夹
        .filter(
          (pageDirName) => this.root.config.ignore.indexOf(pageDirName) === -1
        )
        .forEach((pageDirName) => {
          ...
          page.path = normalize(
            path.join(
              pkg.pagePath.replace(this.root.paths.sourcePath, ''),
              pageDirName,
              'index'
            )
          )
          ...
          const sourceFile = routeConfigSourceFiles.find((sourceFile) => {
            return (
              path.normalize(sourceFile.compilerNode.fileName) ===
          ...
          this.root.log(
            ...
            `Router.${
              page.packageName === 'main' ? '' : page.packageName + '.'
            }${page.method?.name}`
          )
        })
    }
    ...
    if (isExist) {
      if (index !== -1) {
        ...
        this.root.log(
          ...
          `Router.${page.packageName === 'main' ? '' : page.packageName + '.'}${
            page.method?.name
          }`
        )
      } else {
        ...
        page.path = normalize(
          path.resolve(
            pageDirPath.replace(this.root.paths.sourcePath, ''),
            'index'
          )
        )
  ...
  loadRouteConfig(page: Page, configSourceFile?: SourceFile) {
    ...
    configSourceFile.getExportedDeclarations().forEach((declarations, name) => {
      ...
      switch (name) {
        case 'Params':
          routeConfig.params = `import('${path.resolve(
            page.dirPath,
            'route.config'
          )}').Params`
          break
        case 'Data':
          routeConfig.data = `import('${path.resolve(
            page.dirPath,
            'route.config'
          )}').Data`
          break
  ...
  loadMethod(page: Page) {
    ...
    if (
      !routeConfig ||
      (isNil(routeConfig.params) && isNil(routeConfig.data))
    ) {
