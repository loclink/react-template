...
export function getPagesPath(ctx: IPluginContext, options: AutoPathConfig) {
...
export function getTabbarPath(ctx: IPluginContext, options: AutoPathConfig) {
  return path.resolve(
    ...
    `${options.mainPackage.root}/${options.mainPackage.tabbarDir}`
  )
}
...
export function getSubPackagePath(
  ...
  return path.resolve(ctx.paths.sourcePath, options.subPackage.root)
}
...
export async function generateSubPackagePaths(
  ...
  const pagesSubPathArr = fs
    ...
    .map((subName) => {
      const root = `${options.subPackage.root}/${subName}`
      const pages = fs
  ...
  const tabbarPaths = fs
    ...
    .map((item) => {
      ...
      if (fs.pathExistsSync(pageCpnPath)) {
        return {
          text: item,
          pagePath: `${options.mainPackage.root}/${tabbarDir}/${item}/index`,
        }
      }
    })
    .filter((item) => item)
console.log(tabbarDir, 'tabbarDir')
  return {
    custom: true,
    list: tabbarPaths,
  }
...
export async function generateMainPackagePaths(
  ...
  let pagesPathArr = fs
    ...
    .map((item) => {
      ...
      if (fs.pathExistsSync(pageCpnPath)) {
        return `${options.mainPackage.root}/${item}/index`
      }
    })
    .filter((item) => item)
  ...
  console.log(pagesPathArr, 'pagesPathArr')
...
const handleHomePage = async (
  ...
  console.log(pathsPathArr, 'pathsPathArr')
  console.log(homePath, 'homePath')
  const homePathStr = pathsPathArr.find((item) => `/${item}` === homePath)

  console.log(homePathStr, 'homePathStr')
  if (homePathStr) {
